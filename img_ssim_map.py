...
def plot_ssim_map (ssim_map, score, save_to=None):
    """
    Plotea un mapa de calor a partir de un mapa de similitud estructural (SSIM) y muestra el score promedio.

    Args:
        ssim_map (numpy.ndarray): Mapa de similitud estructural.
        score (float): Score promedio SSIM.
        save_to (str, optional): Ruta para guardar la imagen del mapa de calor. Si es None, se muestra en pantalla. Defaults to None.
    """
    ...
    #Si está especificado que se guarde, guardar la imagen, sino mostrarla en pantalla
    if save_to is not None:
        plt.savefig(save_to)
    else:
        plt.show()
...
def _test_single():
    ...
    heatmap = image_ssim_map(ssim_map, save_to=None, mode='rgb')
...
def _test_plot():
    ...
    #plot_ssim_map(ssim_map, score, save_to='heatmap_ssim_plot.png')
    plot_ssim_map(ssim_map, score, save_to=None)
